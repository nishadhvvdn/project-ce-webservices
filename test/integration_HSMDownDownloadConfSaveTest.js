...
describe('Integration Test - HyperSproutManagement/HSMDownDownloadConfSave ', function () {
	this.timeout(45000);
	...
	it('Test Case - 4', function (done) {
		testSession.post('/HSMDownDownloadConfSave')
			.send({ "configName": "HSConfiguration", "SerialNumber": ["000000000000000CELL3_TEST"] })
			.end(function (err, res) {
				var objDet = res.body;
	...
	it('Test Case - 13', function (done) {
		testSession.post('/HSMDownDownloadConfSave')
			.send({ "configName": "HSConfiguration1", "SerialNumber": ["000000000000000CELL3_TEST"] })
			.end(function (err, res) {
				var objDet = res.body;
				console.log(JSON.stringify(objDet));
				expect(objDet).not.to.equal(undefined);
	...
	it('Test Case - 18', function (done) {
		testSession.post('/HSMDownDownloadConfSave')
			.send({ "configName": "Invalid", "SerialNumber": ["000000000HS0001"] })
			.end(function (err, res) {
				var objDet = res.body;
