...
*/
function fetchClientsdataUsage(data, callback) {
    dbMongoCon.getDb(function (err, db) {
        if (err) {
            ...
            fetchClientsdataUsageFromMongoDB(db.delta_Meters, db.delta_User, db.delta_DeltaLink, data, callback);
    });
};
...
function fetchClientsdataUsageFromMongoDB(meterCollection, userCollection, deltalinkCollection, data, callback) {
    try {
                                ...
                                deltalinkCollection.find({ $and: [{ MeterID: consumer[0].MeterID }, { "IsMaster": true }] }).toArray(function (err, res) {
                                    if (err) {
                                        ...
                                        if (res.length) {
                                            ...
                                            //let query = `SET @StartDate:=FROM_UNIXTIME(${data.startTime});SET @EndDate:=FROM_UNIXTIME(${data.endTime}); SELECT Deltalink_SerialNumber, User_Mac_Id , User_Hostname, sum(user_Total) FROM deltamart.deltalink_clients_hourly where ReadTimestamp >= @StartDate and ReadTimestamp < @EndDate and Deltalink_SerialNumber =  '${res[0].DeltalinkSerialNumber}' and DataType = 'User' group by User_Mac_Id , User_Hostname having sum(user_Total) > 0 order by sum(user_Total) desc`
                                            let query = `SET @StartDate:=FROM_UNIXTIME(${data.startTime});SET @EndDate:=FROM_UNIXTIME(${data.endTime});select Deltalink_SerialNumber, User_Mac_Id,
                                            ...
                                            FROM deltamart.deltalink_clients_hourly
                                            where ReadTimestamp >= @StartDate and ReadTimestamp < @EndDate
                                            ...
                                            //             ) cnt ;`
                                            let countQuery = `SET @StartDate:=FROM_UNIXTIME(${data.startTime}); SET @EndDate:=FROM_UNIXTIME(${data.endTime}); select count(*) from ( SELECT User_Mac_Id , User_Hostname, sum(user_Total) 
                                        FROM deltamart.deltalink_clients_hourly where ReadTimestamp >= @StartDate and ReadTimestamp < @EndDate and Deltalink_SerialNumber = '${res[0].DeltalinkSerialNumber}' and DataType = 'User' group by User_Mac_Id , User_Hostname having sum(user_Total) > 0 ) count`
