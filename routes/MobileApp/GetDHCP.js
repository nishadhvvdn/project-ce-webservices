...
router.get('/', function (req, res) {
    try {
        ...
        let Consumserdata = { ConsumerID };
        let dashboardSchema = schema.dashboard;

        authenticateJWT.authentication(req, function (err, result) {
            if (result) {
                schemaValidation.validateSchema(Consumserdata, dashboardSchema, function (err, result) {
                    if (err) {
                        res.json({
                            "data": {},
                            "response": {
                        ...
                        dbCmd.FetchDhcpDetails(ConsumerID, function (err, data) {
                            if (err) {
                                res.json({
                                    "data": {},
                                    "response": {
