...
router.post('/', function (req, res) {
    try {
        if (_.isEmpty(req.body)) {
            ...
            const clearLogsStatusSchema = schema.clearLogsStatusSchema;
            schemaValidation.validateSchema(data, clearLogsStatusSchema, function (err, result) {
                if (err) {
                    res.json({
                        "type": false,
                        "Message": "Invalid Request, Please try again after some time !!",
                    ...
                    if (data.Status === "Success") {
                        data.Status = "Device Log Clear Successful";
                    } else {
                        data.Status = "Device Log Clear Unsuccessful";
                    }
                    dbCmd.saveDeviceClearLogStatus(data, function (err, result) {
                        if (err) {
                            res.json({
                                "Type": false,
                                "Message": err,
